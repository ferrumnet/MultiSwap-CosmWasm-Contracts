...
# cudos-noded tx wasm instantiate 1 '{"owner":"cudos1jlm87kyvr668u5d593mdlrfuwf0t2z0x3ct9th"}' --from=validator --label "FerrumMultiswap" --chain-id=test --gas=auto --gas-adjustment=1.3 -b=block --keyring-backend=test --admin=$VALIDATOR -y
...
# cudos-noded tx wasm execute $CONTRACT '{"add_signer":{"from":"'$VALIDATOR'","signer":"cudos1jlm87kyvr668u5d593mdlrfuwf0t2z0x3ct9th"}}' --amount=1000stake --from=validator --gas=auto --gas-adjustment=1.3 --chain-id=test -y --keyring-backend=test
# cudos-noded tx wasm execute $CONTRACT '{"remove_signer":{"from":"'$VALIDATOR'","signer":"cudos1jlm87kyvr668u5d593mdlrfuwf0t2z0x3ct9th"}}' --amount=1000stake --from=validator --gas=auto --gas-adjustment=1.3 --chain-id=test -y --keyring-backend=test
